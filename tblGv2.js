...
GTable.attachTable = function(tableSelector, options = {}) {
        try {
                ...
                if(!Array.isArray(columns) || columns
                        .length === 0) {
                        throw new Error(
                                "Columns not found. Please attach columns with the table!");
                }
                ...
                if(sorting) {
                        addSortableColumns(table, shortSpec,
                                columns,
                                sortDirection,
                ...
                if(columnsResizing) {
                        addColumnResizing(tableSelector,
                                columns);
                }
                if(columnFiltering) {
                        addColumnFiltering(tableSelector,
                                columns, _data);
                }
                if(headerFilter && columnFiltering == false) {
                        addDataColumnToTableHeading(
                                tableSelector, table,
                                columns);
                        addFilterIcons(table, columns,
                                tableSelector);
                }
                var totalPages = Math.ceil(_data.length /
                        pageSize);
                updateTableChunk(tableSelector, _data,
                        columns, currentPage, pageSize
                        );
                updatePaginationControls(tableSelector,
                        currentPage, totalPages,
                        _data, pageSize, columns,
                        showingentry, groupByColumn,
                        selectedLanguage);
                var totalRecordsCountDiv = '';
                if(totalCount) {
                        totalRecordsCountDiv = document
                                .createElement('div');
                        totalRecordsCountDiv.classList.add(
                                'total-records-count');
                        table.parentNode.appendChild(
                                totalRecordsCountDiv);
                }
                if(paginationOptions || goto_page.on) {
                        if(goto_page.on === false &&
                                paginationOptions != true) {
                                throw new Error(
                                        "please enable goto page for working eith this")
                        }
                        else if(paginationOptions) {
                                addPaginationOptions(table,
                                        pageSize,
                                        tableSelector,
                                        _data,
                                        columns,
                ...
                if(OnKeyBoardShortCuts.enable) {
                        enableKeyboardShortcuts(tableSelector,
                                OnKeyBoardShortCuts
                                .enterPress.colnm_ind,
                                OnKeyBoardShortCuts
                                .enterPress.target,
                                OnKeyBoardShortCuts
                                .enterPress.on)
                ...
                if(rowHighlight.enable || dragAnd_DropRow) {
                        findRows_addClass(dragAnd_DropRow,
                                tableSelector);
                }
                if(HeaderTooltip.on) {
                        addToolTipToHeaders(tableSelector,
                                HeaderTooltip);
                }
                if(context_menu.enable) {
                        contextMenu_workProgrss(context_menu
                                .buttons.edit ?
                                ...
                                languageNoDataAvailabemsg
                                .lang_code : 'en');
                        showContextFirstStep(table,
                                context_menu.buttons
                                .CopyWithHeader,
                                context_menu.buttons
                                .copyRow,
                                tableSelector, columns
                                )
                }
                if(importCsv.enable) {
                        importCSVFile();
        ...
        try {
                ...
                toltipHeader.forEach(function(header, Hindex) {
                        if(tltP.htoolTipin && tltP
                                ...
                                var tooltipText = tltP
                                        .htoolTipinText
                                        .Text[
                                                tooltipIndex];
                                if(tltP.htoolTipinText
                                        .on &&
                                        tooltipText) {
                ...
                console.error("An error occurred while adding tooltips:",
                        error);
        }
}
...
function destroyTable(tableSelector) {
        ...
        if(table) {
                ...
                var paginationa_Option = document.getElementById(
                        "pagingOptions");
                if(paginationa_Option) {
                        paginationa_Option.innerHTML = '';
                }
                var psgination = document.getElementById(
                        "PaginationContainer");
                if(psgination) {
                        psgination.innerHTML = '';
                }
                var shoring_entery_test = document.querySelector(
                        "showing-entry-text");
                if(shoring_entery_test) {
                        shoring_entery_test.innerHTML = '';
                }
                var totalRecordsCount = document.querySelector(
                        "total-records-count");
...
function importCSVFile() {
        ...
        if(importCsv.textStyle != null && importCsv.textStyle != "" &&
                importCsv.textStyle.count > 0) {
...
function showContextFirstStep(table, cpWhd, _copRow, tableSelector,
        columns) {
...
function contextMenu_workProgrss(edit, _del, _cpR, _cpWh, Pn_row,
        exportJson, selectedLanguage) {
        ...
        if(_cpR.on && _cpR.text != '') {
                try {
                        if(_cpR.tooltip.on && _cpR.tooltip.text !=
                                '' && _cpR.tooltip) {
                                try {
                                        copyRowItem = document
                                                .createElement('li');
                                        copyRowItem.id = 'cropRow_';
                                        copyRowItem.textContent = _cpR
                                                .text + '✂️';
                                        copyRowItem.setAttribute(
                                                'data-tooltip',
                                                _cpR.tooltip
                                                .text);
                                        ...
                                        console.warn(error +
                                                'tooltip is not initlize'
                                                );
                                }
                        }
                        else {
                                copyRowItem = document.createElement(
                                        'li');
                                copyRowItem.id = 'cropRow_';
                                copyRowItem.textContent = _cpR.text +
                                        '✂️';
        ...
        if(_cpWh.on && _cpWh.text != '') {
                try {
                        if(_cpWh.tooltip.on && _cpWh.tooltip.text !=
                                '' && _cpWh.tooltip) {
                                try {
                                        copyRowItemWithHeader =
                                                document
                                                .createElement('li')
                                        copyRowItemWithHeader.id =
                                                'cropRow_Header';
                                        copyRowItemWithHeader
                                                .setAttribute(
                                                        'data-tooltip',
                                        ...
                                        console.warn(error +
                                                "it will work fine if not wirking then you can enable tool tip by chaging context options"
                                                )
                                }
                        }
                        else {
                                copyRowItemWithHeader = document
                                        .createElement('li');
                                copyRowItemWithHeader.id =
                                        'cropRow_Header';
                                copyRowItemWithHeader.textContent =
                                        _cpWh.text + '✂️';
                                contextMenu.appendChild(
                                        copyRowItemWithHeader);
                        }
        ...
        if(Pn_row.on && Pn_row.text != '') {
                try {
                        ...
                        Print_rowItem.textContent = Pn_row.text +
                                '🖨️';
        ...
        if(exportJson.on && exportJson.text != '') {
                try {
                        var exportJsonBtn = document.createElement(
                                'button');
                        exportJsonBtn.textContent = translate(
                                selectedLanguage, "exportJson"
                                )
                        exportJsonBtn.setAttribute('id',
                                'exportJsonBtn_e');
...
function enableDragAndDrop(tableSelector) {
        var table = document.getElementById(tableSelector.substring(
                1)); // Remove '#' from the selector
        if(!table) {
        ...
        table.addEventListener('drop', function(e) {
                e.preventDefault();
                var sourceIndex = parseInt(e
                        .dataTransfer.getData(
                                'text'), 10);
                var targetIndex = e.target.rowIndex;
                if(isNaN(sourceIndex) || isNaN(
                                targetIndex) ||
                        sourceIndex === targetIndex) {
                        return;
                }
                var tbody = table.querySelector(
                        'tbody');
...
function EnableLocalisation(container_id, text_style, tableSelector,
        columns, lang_code) {
        var selectLang = document.createElement("select");
        selectLang.setAttribute("id", "languageSelector");
        selectLang.setAttribute("onchange",
        ...
        if(languageSelectorContainer != null) {
                ...
                try {
                        ...
                        console.warn(
                                "Languages not attached. Please use the language selector. and it will working fine");
                }
        }
        else {
                selectLang.setAttribute('style', 'display:none;');
...
function showEmptyTableMessage(tableSelector, message, columns,
        textStyle) {
        var table = document.querySelector(tableSelector);
        var tbody = table.querySelector('tbody');
        if(!tbody) {
...
function addDataColumnToTableHeading(tableSelector, table,
        columnValues) {
        try {
                ...
                if(thead) {
                        ...
                        if(columnValues && columnValues.length >
                                0) { // Check if columnValues is an array and not empty
                                columnValues.forEach(function(column,
                                        index) {
                                        if(column) {
                                                if(thl[
                                                        index]) { // Make sure there is a corresponding th element
                                                        thl[index]
                                                                .setAttribute(
                                                                        'data-column',
                                                                        column
                                                        ...
                                                        console.error(
                                                                'Column value provided without corresponding th element');
                                                }
                                        }
                                        else {
                                                console.error(
                                                        'Please provide valid column values');
                                        }
                                });
                        }
                        else {
                                console.error(
                                        'Please add column values to proceed');
                        }
                }
        }
        catch (error) {
...
function showFilterModal(columnField, columns, tableSelector) {
        ...
        applyButton.addEventListener('click', function() {
                applyColumnFilter(columnField,
                        filterInput.value,
                        tableSelector
                        ); // Apply the filter
                filterModal.parentNode.removeChild(
                        filterModal);
        });
...
function addGlobalSearch(tableSelector, columns, _data,
        defaultPageSize, showingentry, currentPage, pageSize,
        totalRecordsCountDiv, groupByColumn, selectedLanguage,
        showingEnteriesTextEnable) {
        try {
                var table = document.querySelector(tableSelector);
                ...
                var paginationOptionsContainer = document
                        .createElement('div');
                paginationOptionsContainer.classList.add(
                        'pagination-options');
                paginationOptionsContainer.setAttribute('style',
                        'display:none;')
                ...
                if(showingentry) {
                        try {
                                var showingEntryText = document
                                        .querySelector(
                                                '.showing-entry-text'
                                                );
                                var startEntry = (currentPage - 1) *
                                        pageSize + 1;
                                var endEntry = Math.min(currentPage *
                                ...
                                if(showingEnteriesTextEnable) {
                                        ...
                                        if(selectedLanguage ===
                                                ...
                                                selectedLanguage ===
                                                'ko') {
                                                getShowingEnteryTextFromAnotherPlaceWhenLanguageChange
                                                        = translate(
                                                                selectedLanguage,
                                                                'translationShowEntries'
                                                                );
                                                showingEntryText
                                                        .textContent =
                                                        getShowingEnteryTextFromAnotherPlaceWhenLanguageChange
                                        ...
                                        var getShowingEnteryTextFromAnotherPlace =
                                                translate('en',
                                                        'translationShowEntries'
                                                        )
                                        showingEntryText.textContent =
                                                getShowingEnteryTextFromAnotherPlace
                                                .replace('{}',
                                                        startEntry)
                                                .replace('{}',
                                                        endEntry)
                ...
                paginationOptionsContainer.appendChild(
                        pageSizeContainer);
                var pSelectElement = document.getElementById(
                        'pselect');
                if(pSelectElement) {
                        var pagingOptions = document.getElementById(
                                "pagingOptions");
                        // pagingOptions.classList.add("hide");
                        pSelectElement.parentNode.insertBefore(
                                searchInput, pSelectElement
                                .nextSibling);
                }
                else {
                ...
                table.parentNode.insertBefore(
                        paginationOptionsContainer, table);
                updateTotalRecordsCount(totalRecordsCountDiv, _data
                        .length, selectedLanguage);
        }
        catch (error) {
...
function enableKeyboardShortcuts(tableSelector, _ColumnIndex, target_,
        enter_pressOn) {
        var table = document.querySelector(tableSelector);
        if(!table) {
                console.error('Table element not found.');
        ...
        document.addEventListener('keydown', function(e) {
                ...
                var activeRow = table.querySelector(
                        '.active-row');
                ...
                switch(e.key) {
                        ...
                        case 'Enter':
                                if(enter_pressOn) {
                                                ...
                                                if(columnIndices
                                                        ...
                                                        if(
                                                                ...
                                                                if(target_ !=
                                                                        ...
                                                                        anchorTag
                                                                                .click(); // Trigger click on the anchor tag
                                                                }
                                                        }
                                                }
                                        ...
                                        console.warn(
                                                'for Enter click please on properties of enter button press')
                                }
                        case 'Delete':
                                if(activeRow) {
                ...
                if(['ArrowUp', 'ArrowDown'].includes(e
                                .key)) {
                        e.preventDefault();
                }
        });
...
function applyGlobalSearch(tableSelector, columns, _data) {
        ...
        var rows = tbody.querySelectorAll('tr');
        var matchingRows =
        0; // Initialize a counter for matching rows
        if(rows.length > 1 && _data != '') {
                ...
                if(matchingRows === 0) {
                        showEmptyTableMessage(tableSelector,
                ...
                showEmptyTableMessage(tableSelector, translate(
                        selectedLanguage,
                        'noMatchingRecord'), columns);
        }
}
...
function updateLocalization(selectedLanguage) {
        try {
                var tralslatedTextSearchPlaceHolder = translate(
                        selectedLanguage, 'searchPlaceholder');
                document.getElementById('globalSearch').setAttribute(
                        "placeholder",
                        tralslatedTextSearchPlaceHolder)
        }
        catch (error) {
                console.warn(error,
                        'attribute is working fine if now available please contact us!'
                        )
        }
}
...
function updateAfterLocalization(selectedLanguage) {
        try {
                var editButton = document.getElementById(
                        'edit_bTableBtn');
                editButton.textContent = translate(selectedLanguage,
                        "edit");
                var crpRowHead = document.getElementById(
                        'cropRow_Header');
                crpRowHead.textContent = translate(selectedLanguage,
                        'copyCurrentRowWithHeaders') + '✂️';
                var printRow = document.getElementById('Print_row');
                printRow.textContent = translate(selectedLanguage,
                        'printCurrentRow') + '🖨️';
                var cropRow_ = document.getElementById('cropRow_');
                cropRow_.textContent = translate(selectedLanguage,
                        'copyCurrentRow') + '✂️';
        }
        catch (error) {
                console.log(error);
...
function updateTotalRecordsCount(totalRecordsCountDiv, count,
        selectedLanguage) {
        updateLocalization(selectedLanguage);
        totalRecordsCountDiv.textContent = 'Total Records: ' + count;
}

function addSortableColumns(table, shortSpec, columns, sortDirection,
        currentPage, pageSize, _data, tableSelector, showingentry) {
        addAttributeDataShortable(table.querySelector("thead"),
                shortSpec);
        var thList = table.querySelectorAll(
                'th[data-sortable="true"]');
        thList.forEach(function(th) {
                th.addEventListener('click', throttle(
                        function() {
                                thList.forEach(function(
                                        header) {
                                        header.innerHTML =
                                                header
                                                .innerHTML
...
function updateTable(tableSelector, _data, _columns, currentPage,
        pageSize, groupByColumns) {
        var table = document.querySelector(tableSelector);
        var tbody = table.querySelector('tbody');
        tbody.innerHTML = '';
        ...
        function renderGroupRow(groupKeys) {
                ...
                groupCell.textContent = groupKeys.join(
                ' - '); // Display group keys
...
function updatePaginationControls(tableSelector, currentPage,
        totalPages, _data, pageSize, columns, showingentry,
        groupByColumn, selectedLanguage) {
        try {
                ...
                if(paginationContainer) {
                        ...
                        var backButton = createPaginationButton('<',
                                currentPage - 1,
                                currentPage === 1);
                        var forwardButton = createPaginationButton(
                                '>', currentPage + 1,
                                currentPage === totalPages);
                        var prev2Button = createPaginationButton('<<',
                                currentPage - 2,
                                currentPage <= 2);
                        var next2Button = createPaginationButton('>>',
                                currentPage + 2,
                                currentPage >= totalPages - 1);
                        var currentPageIndicator = document
                                .createElement('span');
                        ...
                        if(showingentry) {
                                try {
                                        ...
                                        if(showingEntryText == null) {
                                                ...
                                                showingEntryText
                                                        .textContent =
                                                        `Showing ${startEntry}-${endEntry} of ${_data.length} entries`;
                                        }
                                }
                                catch (error) {
                                        console.log(error)
...
function addColumnFiltering(tableSelector, columns, _data,
        currentPage, pageSize) {
        var table = document.querySelector(tableSelector);
        var thead = table.querySelector('thead');
        if(thead) {
                ...
                var
        filterInputs = []; // Array to store filter input elements
                                ...
                                input.addEventListener(
                                        ...
                                        function() {
                                                applyFilters
                                                        (tableSelector,
                                                                columns
                                                                ); // Call applyFilters with _data argument
                                        });
...
function applyFilters(tableSelector, columns) {
        var table = document.querySelector(tableSelector);
        var filters = table.querySelectorAll(
...
function addPaginationOptions(table, defaultPageSize, tableSelector,
        _data, columns, totalRecordsCountDiv, cPZise, showingentry,
        goto_page, groupByColumn, selectedLanguage) {
        try {
                var paginationOptionsContainer = document
                        .createElement('div');
                paginationOptionsContainer.classList.add(
                        'pagination-options');
                paginationOptionsContainer.setAttribute("id",
                        'pagingOptions');
                if(cPZise == false) {
                        var pageSizeLabel = document.createElement(
                                'label');
                        pageSizeLabel.textContent = 'Page Size: ';
                        var pageSizeSelect = document.createElement(
                                'select');
                        ...
                        var pageSizeContainer = document
                                .createElement('div');
                        pageSizeContainer.appendChild(pageSizeLabel);
                        pageSizeContainer.appendChild(pageSizeSelect);
                        paginationOptionsContainer.appendChild(
                        ...
                        var customPageSizeInput = document
                                .createElement('input');
                        customPageSizeInput.setAttribute('type',
                                'number');
                        customPageSizeInput.setAttribute('min', '1');
                        customPageSizeInput.setAttribute(
                                'placeholder', 'Custom');
                        var applyPageSizeButton = document
                                .createElement('button');
                        applyPageSizeButton.textContent = 'Apply';
                        applyPageSizeButton.addEventListener('click',
                                function() {
                                        var newPageSize =
                                                parseInt(
                                                        customPageSizeInput
                        ...
                        var customPageSizeContainer = document
                                .createElement('div');
                        customPageSizeContainer.appendChild(
                                customPageSizeInput);
                        customPageSizeContainer.appendChild(
                                applyPageSizeButton);
                        paginationOptionsContainer.appendChild(
                                customPageSizeContainer);
                }
                if(showingentry) {
                        var showingEntryText = document.createElement(
                                'div');
                        showingEntryText.classList.add(
                                'showing-entry-text');
                        paginationOptionsContainer.appendChild(
                                showingEntryText);
                        var paginationcontrols = document
...
function createShowingEntryAgain() {
        var paginationOptionsContainer = document.getElementById(
                "pagingOptions");
        if(paginationOptionsContainer != null) {
                try {
                        ...
                        console.warn(
                                "pagination container is missing please enable pagination")
                }
        }
        else {
                console.warn(
                        "if working than don't do anything this will work fine everytime!")
        }
}
...
function addColumnResizing(tableSelector, columns) {
        ...
        document.addEventListener('mousemove', function(e) {
                if(resizingColumn) {
                        var width = resizingColumn
                                .offsetWidth + (e
                                        .pageX -
                                        startX);
                        if(width > 50) {
                                resizingColumn.style
                                        .width =
                                        width + 'px';
                        }
                        startX = e.pageX;
...
function renderDeleteButton(rowIndex) {
        return '<button class="delete-btn" data-row-index="' +
                rowIndex + '">Delete</button>';
}

function renderSelectCheckbox(data, row) {
        return '<input type="checkbox" class="select-row-checkbox" data-row-index="' +
                row.index + '">';
}
document.addEventListener('click', function(event) {
        var target = event.target;
        if(target.classList.contains('delete-btn')) {
                var rowIndex = parseInt(target
                        .getAttribute(
                                'data-row-index'
                                ));
                if(!isNaN(rowIndex)) {
...
function updateRowIndexes() {
        ...
        rows.forEach(function(row, index) {
                var deleteBtn = row.querySelector(
                        '.delete-btn');
                if(deleteBtn) {
...
function updateTableChunk(tableSelector, _data, _columns, currentPage,
        pageSize) {
        var table = document.querySelector(tableSelector);
        var tbody = table.querySelector('tbody');
        ...
        var startIndex = (currentPage - 1) * pageSize;
        var endIndex = startIndex + pageSize;
        for(var i = startIndex; i < endIndex && i < _data
                .length; i++) {
                var item = _data[i];
                var newRow = document.createElement('tr');
                _columns.forEach(function(column) {
                        var newCell = document
                                .createElement('td');
